...
import { buildHhcoHistoryDetailPath, paths } from '@/app/router/paths';
      ...
      >
        ...
        <div className="space-y-2">
          <h1 className="font-display text-2xl font-semibold tracking-tight">
            {t('hhcoOps.completed.title')}
          </h1>
          <p className="text-sm text-muted-foreground">{t('hhcoOps.completed.description')}</p>
        </div>

        <dl className="grid gap-3 text-left text-sm">
          <div className="rounded-xl bg-muted/40 px-3 py-2.5">
            <dt className="text-xs uppercase tracking-wide text-muted-foreground">
              {t('hhcoOps.completed.vehicle')}
            </dt>
          ...
          {multiDay ? (
            <div className="rounded-xl bg-muted/40 px-3 py-2.5">
              <dt className="text-xs uppercase tracking-wide text-muted-foreground">
                {t('hhcoOps.continue.numberOfDays')}
              </dt>
              <dd className="mt-1 font-medium text-foreground">
          ...
          <div className="rounded-xl bg-muted/40 px-3 py-2.5">
            <dt className="text-xs uppercase tracking-wide text-muted-foreground">
              {t('hhcoOps.completed.workingHours')}
            </dt>
            <dd className="mt-1 font-medium text-foreground">
          ...
          <div className="rounded-xl bg-muted/40 px-3 py-2.5">
            <dt className="text-xs uppercase tracking-wide text-muted-foreground">
              {t('hhcoOps.completed.syncStatus')}
            </dt>
            <dd className="mt-1">
              <Badge variant={synced ? 'success' : 'warning'} className="rounded-md">
                {synced
                  ? t('hhcoOps.completed.syncSynced')
                  : t('hhcoOps.completed.syncWaiting')}
              </Badge>
            </dd>
          </div>
        </dl>
        ...
        <div className="flex flex-wrap gap-3 pt-1">
          ...
          <Button asChild variant="secondary">
            <Link to={buildHhcoHistoryDetailPath(session.id)}>
              {t('hhcoOps.completed.viewOperation')}
            </Link>
          </Button>
        </div>
      </Card>
