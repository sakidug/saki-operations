...
export async function listPreviousHhcoDeliveries(
  employeeId: string,
...
export function matchesHistorySearch(
  ...
  const hireRaw =
    typeof session.customFields.dealerId === 'string'
      ? session.customFields.dealerId.toLowerCase()
      : '';
...
export function buildOperationTimeline(session: OperationsSession): TimelineEvent[] {
  const events: TimelineEvent[] = [
    {
      id: 'started',
      kind: 'started',
      titleKey: 'hhcoOps.history.timeline.started',
  ...
  if (session.startOdometer != null) {
    events.push({
      id: 'start_odometer',
      kind: 'start_odometer',
      titleKey: 'hhcoOps.history.timeline.startOdometer',
      timestamp: session.startTime,
  ...
  if (isMultiDaySession(session)) {
    ...
    for (const day of days) {
      events.push({
        id: `day_${day.day}`,
        kind: 'day',
        titleKey: 'hhcoOps.history.timeline.day',
  ...
  if (session.endOdometer != null && !isMultiDaySession(session)) {
    events.push({
      id: 'end_odometer',
      kind: 'end_odometer',
      titleKey: 'hhcoOps.history.timeline.endOdometer',
  ...
  events.push({
    id: 'completed',
    kind: 'completed',
    titleKey: 'hhcoOps.history.timeline.completed',
...
export function evidenceLabelKey(type: string): string {
  if (type === 'start_odometer') return 'hhcoOps.history.photos.startOdometer';
  if (type === 'end_odometer') return 'hhcoOps.history.photos.endOdometer';
  if (type === 'start_time') return 'hhcoOps.history.photos.startTime';
  if (type === 'end_time') return 'hhcoOps.history.photos.endTime';
  if (type === 'delivery_photo') return 'hhcoOps.history.photos.delivery';
  if (/^day_\d+_start_time$/.test(type)) return 'hhcoOps.history.photos.dayStartTime';
  if (/^day_\d+_end_time$/.test(type)) return 'hhcoOps.history.photos.dayEndTime';
  return 'hhcoOps.history.photos.generic';
}
