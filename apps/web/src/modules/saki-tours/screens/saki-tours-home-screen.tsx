 ...
 */
export function SakiToursHomeScreen() {
  const { t } = useAppTranslation();
  const { user } = useSession();
  const reduceMotion = useReducedMotion();
  const { session: activeSession, loading } = useActiveToursSession(user?.employeeId);
      ...
      <div data-brand="tours" className="contents">
        <header className="space-y-2">
          ...
          <h1 className="font-display text-2xl font-semibold tracking-tight sm:text-3xl">
            {t('toursOps.home.title')}
          </h1>
          <p className="max-w-xl text-sm text-muted-foreground sm:text-base">
            {t('toursOps.home.description')}
          </p>
        </header>
        ...
        <motion.nav
          aria-label={t('toursOps.home.actionsRegion')}
        ...
        >
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                <span className="block text-base font-semibold text-foreground">
                  {t('toursOps.home.startNew')}
                </span>
                <span className="mt-0.5 block text-sm text-muted-foreground">
                  {t('toursOps.home.startNewHint')}
                </span>
              </span>
              <ArrowRight
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="block text-base font-semibold text-foreground">
                {t('toursOps.home.viewPrevious')}
              </span>
              <span className="mt-0.5 block text-sm text-muted-foreground">
                {t('toursOps.home.viewPreviousHint')}
              </span>
            </span>
            <ArrowRight
              className="size-5 shrink-0 text-muted-foreground transition-transform group-hover:translate-x-0.5"
