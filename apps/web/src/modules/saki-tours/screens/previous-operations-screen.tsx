...
export function PreviousOperationsScreen() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setLoading(true);
      try {
        const rows = await listPreviousToursOperations(user.employeeId);
      ...
      <div data-brand="tours" className="contents">
        <header className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-1">
            ...
            <h1 className="font-display text-2xl font-semibold tracking-tight sm:text-3xl">
              {t('toursOps.history.title')}
            </h1>
            <p className="max-w-xl text-sm text-muted-foreground">
              {t('toursOps.history.description')}
            </p>
          </div>
          <Button asChild variant="secondary" size="sm">
            <Link to={paths.sakiTours}>{t('toursOps.history.backHome')}</Link>
          </Button>
        </header>
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            title={t('toursOps.history.emptyTitle')}
            description={t('toursOps.history.emptyDescription')}
          ...
          <>
            ...
            {filtered.length === 0 ? (
              <EmptyState
                icon={<HistoryIcon aria-hidden />}
                title={t('toursOps.history.filterEmptyTitle')}
                description={t('toursOps.history.filterEmptyDescription')}
                className="glass min-h-[12rem] border-solid"
                actionLabel={filtersActive ? t('toursOps.history.clearFilters') : undefined}
                onAction={filtersActive ? () => setFilters(DEFAULT_FILTERS) : undefined}
              />
            ) : (
              <ul className="flex flex-col gap-3" aria-label={t('toursOps.history.listRegion')}>
