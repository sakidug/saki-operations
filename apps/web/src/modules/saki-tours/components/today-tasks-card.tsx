...
function taskLabelKey(id: DayTaskId): string {
  switch (id) {
    case 'start_work_time':
      return 'toursOps.multiDay.tasks.startWorkTime';
    case 'start_odometer':
      return 'toursOps.multiDay.tasks.startOdometer';
    case 'end_work_time':
      return 'toursOps.multiDay.tasks.endWorkTime';
    case 'end_odometer':
      return 'toursOps.multiDay.tasks.endOdometer';
    case 'finish':
      return 'toursOps.multiDay.tasks.finish';
    default:
      return 'toursOps.multiDay.tasks.startWorkTime';
  }
}
    ...
    <Card variant="glass" padding="lg" className={cn('space-y-4', className)}>
      <div>
        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
          {final ? t('toursOps.multiDay.finalDay') : t('toursOps.multiDay.dayLabel', { day })}
        </p>
        <h2 className="mt-1 font-display text-lg font-semibold tracking-tight">
          {t('toursOps.multiDay.todaysTasks')}
        </h2>
      </div>

      <ul className="space-y-2.5" aria-label={t('toursOps.multiDay.todaysTasks')}>
        {tasks.map((task) => (
          <li
            key={task.id}
            className={cn(
          ...
          >
            ...
            <span className="sr-only">
              {task.done ? t('toursOps.multiDay.taskDone') : t('toursOps.multiDay.taskPending')}
            </span>
          </li>
        ))}
      </ul>
