    ...
    <>
      <Card variant="glass" padding="lg" className={cn('space-y-4', className)}>
        <h2 className="font-display text-lg font-semibold tracking-tight">
          {t('toursOps.history.photos.title')}
        </h2>
        ...
        <p className="text-xs text-muted-foreground">{t('toursOps.history.photos.readOnly')}</p>
      </Card>

      <Dialog open={Boolean(active)} onOpenChange={(open) => !open && setActive(null)}>
        <DialogContent
          closeLabel={t('toursOps.history.photos.close')}
