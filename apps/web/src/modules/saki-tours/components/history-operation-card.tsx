...
import { buildSakiToursHistoryDetailPath } from '@/app/router/paths';
...
export function HistoryOperationCard({ session, index = 0 }: HistoryOperationCardProps) {
  ...
  return (
    ...
    >
      <Link
        to={buildSakiToursHistoryDetailPath(session.id)}
        className={cn(
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 space-y-1">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
              {multiDay
                ? t('toursOps.history.card.multiDay')
                : t('toursOps.history.card.singleDay')}
            </p>
        ...
        <dl className="grid gap-2 text-sm sm:grid-cols-2">
          <div>
            <dt className="sr-only">{t('toursOps.history.card.vehicle')}</dt>
            <dd className="font-medium text-foreground">{getSessionVehicleName(session)}</dd>
          </div>

          {multiDay ? (
            <div>
              <dt className="sr-only">{t('toursOps.continue.numberOfDays')}</dt>
              <dd className="text-muted-foreground">
                {t('toursOps.history.card.daysCount', { count: days })}
              </dd>
            </div>
          ) : (
            <div>
              <dt className="sr-only">{t('toursOps.history.card.timeRange')}</dt>
              <dd className="text-muted-foreground">
          ...
          <div>
            <dt className="sr-only">{t('toursOps.history.card.hours')}</dt>
            <dd className="font-medium text-foreground">
              {formatWorkingHoursLabel(getSessionWorkingMs(session))}
            </dd>
          </div>
          <div>
            <dt className="sr-only">{t('toursOps.history.card.km')}</dt>
            <dd className="font-medium text-foreground">
