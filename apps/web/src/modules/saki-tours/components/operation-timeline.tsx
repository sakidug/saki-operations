  ...
  return (
    <Card variant="glass" padding="lg" className="space-y-4">
      <h2 className="font-display text-lg font-semibold tracking-tight">
        {t('toursOps.history.timeline.title')}
      </h2>
                  ...
                  <>
                    ...
                    <div className="rounded-xl bg-muted/40 px-3 py-2 text-sm">
                      <p className="text-muted-foreground">
                        {t('toursOps.history.timeline.startTime')}:{' '}
                        <span className="font-medium text-foreground">
                          {formatOperationTime(event.startTime, locale)}
                        </span>
                      </p>
                      <p className="mt-1 text-muted-foreground">
                        {t('toursOps.history.timeline.endTime')}:{' '}
                        <span className="font-medium text-foreground">
                      ...
                      {event.showEndOdometer && endOdometer != null ? (
                        <p className="mt-1 text-muted-foreground">
                          {t('toursOps.history.timeline.endOdometer')}:{' '}
                          <span className="font-medium text-foreground">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {event.kind === 'completed' ? (
                        <Badge variant="success" className="rounded-md">
                          {t('toursOps.history.timeline.done')}
                        </Badge>
                      ) : null}
                    </div>
                    {event.timestamp ? (
                    ...
                    {event.subtitle ? (
                      <p className="text-sm font-medium text-foreground">
                        {event.kind === 'start_odometer' || event.kind === 'end_odometer'
                          ? `${formatKm(Number(event.subtitle), locale)} ${t('toursOps.odometer.unit')}`
                          : event.subtitle}
                      </p>
                    ) : null}
                  </>
